...
Git 通常使用 HTTPS 协议进行传输，但 HTTPS 协议在访问 GitHub 容易出现连接不稳定的问题，此时切换为 SSH 协议更加稳定。SSH 本是一种传输协议，这里将其放在版本控制的章节中，是因为 git 托管可使用 SSH 协议进行传输，这里就合并在一起了。

而一般的 Git 自带 SSH 客户端，无需额外安装，下面我们来实现 Git 中的 SSH 传输。
