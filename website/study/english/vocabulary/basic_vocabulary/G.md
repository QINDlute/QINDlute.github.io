---
date: 2026-2-13
word: true
---

# 词根 G
## gest
*gest*=携带、做动作

- gesture n. 手势；姿势 v. 做手势
- gestation n. 怀孕；酝酿
- gestational adj. 妊娠的
- gestate v. 孕育；构思
- suggest v. 建议；提出；暗示
- suggestive adj. 暗示的；影射的
- suggestion n. 建议；提议
- digest n. 摘要；文摘 v. 消化；领会
- ingest v. 咽下；摄取；吸收
- congest v. 充血；充满；使拥挤
- register [n.][v.]=登记；注册


## graph=gram
*graph=gram*=写、画、图表（write, draw）

- graph n. 图表；曲线图
- graphic adj. 生动的；图解的 n. 图标
- geography n. 地理；地理学；地形 <interval>`geo-`+graphy=地球+写→描述地球</interval>
- biography n. 传记 <interval>`bio-`+graphy=生命+写→写人生</interval>
- paragraph n. 段落 <interval>`para-`+graph=旁边+段落=写文章</interval>
- photograph n. 照片 v. 拍照
- gram n. 克
- program n. 程序；计划；节目
- diagram n. 图表；图解 <interval>`dia-`+gram=穿过+绘制→绘制图表</interval>
- telegram n. 电报 <interval>`tele-`+gram=远+写=远程通信</interval>
- grammar n. 语法


## gress=grad
*gress=grad*=迈步、走（step, go）

- progress [n.][v.] 进步；进展；前进
- progressive adj. 先进的
- progression n. 进展；发展
- regress [n.][v.] 倒退；退化；退步
- egress n. 外出；出口；
- ingress n. 进入；进入权
- aggress v. 攻击；侵犯 <interval>`ag-`+gress=向+走=向你狂奔而来→来打你</interval>
- aggressor n. 侵虐者；攻击者
- congress n. 国会；议会；代表大会 <interval>`con-`+gress=一起+走=从各地汇聚到一起来开会</interval>

<marker-text><em>gress</em> 走→<em>grad</em> 等级</marker-text>

>: 一步一台阶实现阶级跨越

- grade n. 等级；年级；分数；程度 v. 给……评分；分等
- gradual adj. 逐渐的；渐进的
- graduate v. 毕业；授予学位；获得 n. 毕业生 adj. 研究生的
- gradation n. 分级；层次；渐变
- degree n. 学位；程度；等级；度数


## gene=gen
1. <Badge type="warning">生育、产生</Badge>
- generate v. 引起；产生；导致
- hydrogen n. 氢 <interval>`hydro-`💧+gen=水+产生=产生水的物质（氢气燃烧会生成水）</interval>
- oxygen n. 氧气
- degenerate v. 退化；堕落 adj. 退化的
- generation n. 一代；产生
- generous adj. 慷慨的；丰富的 <interval>`gene-`+rous=产生+adj.=产出丰富、量大管饱</interval>
- genesis n. 创始；起源
- engender v. 使发生；引起
- germinate v. 使产生；使发芽或发育
- progeny n. 子孙后代
- progenitor n. 祖先；创始人；先驱

2. 天生、遗传🧬
- gene n. 基因
- genetic adj. 遗传的，基因的
- congenital adj. 先天的，天生的
- gentle adj. 温和的；文雅的 <interval>”与生俱来的贵族气质“</interval>
- genius n. 天才；天赋
- ingenious adj. 机灵的；有独创性的

>: 特殊引申：天生才能→运用才智去发明设计
>: - engine n. 发动机；引擎；火车头 <interval>由才智产生的装置</interval>
>: - engineer n. 工程师；机械师 v. 设计；建造 <interval>运用才智去设计的人</interval>

- ingenuous adj. 天真的；坦率的
- genuine adj. 真正的；真诚的

3. 种族、种类🧜‍♀️
>: 从 “生育” 引申为 “由共同祖先繁衍而来的群体”，进而发展为 “分类、种类”。

- general adj. 普遍的；总体的 n. 将军
- generic adj. 一般的；属的
- gender n. 性别
- genus n. 属；类
- heterogeneous adj. 异质的；不同种类的
- homogeneous adj. 同质的，同类的
- indigenous adj. 本土的，土著的


## gra
*gra*=赞美、恩惠、愉悦

- agree v. 同意；一致
- grace n. 优雅；恩宠 v. 为……增色；使更加优美
- disgrace n. 耻辱；失宠 v. 使丢脸
- gracious adj. 和蔼的；亲切的；仁慈的
- grateful adj. 感激的；感謝的
- gratitude n. 感谢；感激之情
- gratulate v. 祝贺；表示欣喜
- congratulation n. 恭喜；祝贺
- ingrate n. 忘恩负义之人 <interval>`in-`+grate=否定+感恩=忘恩负义</interval>
- gratify v. 使满足；使高兴
- ingratiate v. 讨好；逢迎 <interval>`in-`+grat+iate=进入+愉悦+v.=进入他人的愉悦状态</interval>


## gno=gn=quaint
1. *gno=gn=quaint*=知道（know）
- know v. 知道；了解；认识 <interval>格林规律：g浊→k清</interval>
- knowledge n. 知识；了解；学问
- acknowledge v. 承认；感谢；认可
---
- <gdt>co-</gdt> 加强
    - cognize v. 认知；认识 & `co-` 强调+gn 知道+`-ize` v.=彻底知道→认识
    - recognize v. 认出；识别；承认 & `re-` 再次+cognize 认识=再次认识→认出
    - recognition n. 识别；承认；认可
    - cognizant adj. 认识的；知晓的
    - cognizance n. 认识；察觉
    - cognitive adj. 认知的；认识的
    - cognition n. 认识；认知；感知
    - incognito adj. 匿名的 ad. 伪装地、隐姓埋名地
---
- <gdt>i-</gdt> 否定
    - ignore v. 忽视；不理睬 & `i-` 否定+gno 知道+re=不（主动）知道→忽视
    - ignorant adj. 无知的；缺乏知识的
    - ignorance n. 无知；愚昧；不知道
    - agnostic n. 不可知论者 adj. 不可知论的；怀疑的
---
- <gdt>dia-</gdt> 贯穿、加强语气
    - diagnose v. 诊断；判断 & `dia-` 加强语气+gno 知道+se=彻底了解病情→诊断
    - diagnosis n. 诊断；诊断书
---
- <gdt>pro-</gdt> 向前
    - prognosis n. 预测；预后；预断病情 & `pro-`+gnosis=向前+知道=向前预断病情
    - prognosticate v. 预测；语言；预示
---
- gnosis n. 真知；灵知；诺斯替派 & “无所不知的智慧”
- gnome n. 格言；侏儒；地精 <interval>“知晓很多秘密的精灵”→地精</interval>
- gnomic adj. 格言的
- physiognomy n. 相貌；面貌；地貌 & `physio-` 自然+gno 知道+my=通过自然外观而知道→面相
- quaint adj. 古色古香的；古怪的；离奇有趣的 & 
- acquaint v. 使熟悉；使了解；使认识 & `ac-` 去+quaint 知道=想要去了解
- acquaintance n. 熟人；了解；相识 & 了解的很透彻的人→熟人

::: tip 格林规律{#green_law}
格林定律是由德国语言学家 Jacob Grimm 提出的音变规律，它描述了原始印欧语（PIE）中的辅音在进入日耳曼语族（包括英语、德语、荷兰语等）时，发生的系统性、有规律的清浊音变化。

| **PIE 辅音** | **日耳曼语对应** | **例子** |
| :---------: | :------------: | :------: |
| 清塞音：**p**, **t**, **k** | 清擦音: **f**, **θ**, **h** | pater (父亲) → 英语 **father** (p→f)；trei (三) → 英语 **three** (t→θ)；ḱard- (心) → 英语 **heart** (k→h) |
| 浊塞音：**b**, **d**, **g** | 清塞音: **p**, **t**, **k** | bʰreh₂tér (兄弟) → 英语 **brother** (b→p？不，这里是浊送气音，看第三组)；dōm- (房子) → 英语 **tomb** (d→t)；ǵnō- (知道) → 英语 **know** (g→k) |
| 浊送气塞音: **bʰ**, **dʰ**, **gʰ** | 浊塞音: **b**, **d**, **g** | bʰreh₂tér (兄弟) → 英语 **brother** (bʰ→b)；dʰeh₃- (做) → 英语 **do** (dʰ→d)；gʰrem- (咆哮) → 英语 **grim** (gʰ→g) |
:::


## grav
1. <Badge type="warning"><em>grav</em></Badge>=重

- gravity n. 重力；庄重；严重性
- gravitate v. 被吸引；受引力作用；移向
- gravimeter n. 重力仪
- grave n. 坟墓；死亡 adj. 严肃的；严重的
- gravid adj. 怀孕的
- aggravate v. 使恶化；加剧；激怒
- aggravation n. 恶化；加剧；激怒

2. *griev*=悲伤、痛苦😢
>: 由“重”引申为“心情沉重（被压得喘不过来气）”
- grief n. 悲痛；悲伤
- grieve v. 使伤心；悲痛
- aggrieve v. 侵害；使委屈；使苦恼


## gymno
gymno=裸露的，赤裸的 :muscle:

- gymnasium n. 健身房；体育馆 <interval>复数形式：gymnasia "光膀子💦健身"</interval>
- gymnastic adj. 体操的；体操训练的
- gymnastics n. 体操；体操训练
- gymnast n. 体操运动员
- gymnosperm n. 裸子植物
- gymnospermous adj. 裸子植物的
- Gymnophiona n. 无足目


## gard=guard=ward💂🏻
*gard=guard=ward*=看管、守卫、注意

1. 看管
- ward n. 病房；行政区；选区 v. 监护；守护；挡住
- warden n. 监狱长；看守人；督察员
- warder n. 监狱看守；守卫
- wardrobe n. 衣柜；衣橱；行头 & ward 看管+robe 衣服=看管衣物的地方→衣柜
- award v. 授予；颁奖；裁定 n. 奖品；裁定；奖学金 & `a-` 朝向+ward 看管→看管任务完成后给予奖励
- reward n. 报酬；奖励 v. 奖励；回报；奖赏
- steward n. 乘务员；管家；守护者；保护者
- stewardess n. 女乘务员；空姐
- stewardship n. 管理工作；管理

2. 注意
- regard v. 注视；认为；看待；凝视 n. 尊敬；问候；关心 & `re-` 再+gard 注意=反复注意看→注视、尊重
- regarding prep. 关于；至于 v. 认为；看待
- regardless adj. 不注意的；不顾的 ad. 不顾；不加理会
- disregard \[v.]\[n.] 忽视；漠视

3. 守卫
- guard n.警卫；保卫 v. 守卫；警戒；防范
- guardian n. 监护人；捍卫者；保护者
- unguarded adj. 无防卫的；不谨慎的；无人守护的
- vanguard n. 前锋；先驱；先头部队
- bodyguard n. 保镖
- safeguard v. 保护；维护；捍卫 n. 保护措施


## 其他
- globe n. 地球；球体；世界
- grain n. 谷物；颗粒；谷类；粮食
- grand adj. 重大的；壮丽的；总的 n. 一千美元（或英镑）；大钢琴
- gross adj. 总的；粗俗的；毛的；显而易见的 v. 总额
- grocer n. 杂货商；食品商
- grocery n. 食品杂货店 v. 购物
- gather v. 聚集；收集；推断
- together ad. 一起；共同；集拢地
- greedy adj. 贪婪的 <interval>形似 green：绿色哥布林对金币很贪婪，绿色象征贪婪</interval>
- guidance n. 指导；引导；导航
- guilty adj. 有罪的；内疚的
- government n. 政府；内阁；政治；管辖
